#Name: Gillian Mary O'Donnell
#Student ID: 120394016
#Programme: First Year Data Science and Analytics
...
def frame(w, h, c):
    #Your code goes here
    #must have gap and use for loop
    #If the width or height is 2 or less you should return the string Invalid.
    if w <=2 or h <= 2: #If the width or height is 2 or less you should return the string Invalid.
        return Invalid
    for firstHoriszontalChar in range(1,w+1): #for loop to create rectangel #firstHorizontalChar is the variable for printing the width characters and the top of the frame # outer for loop iterating for 1 to width+1 other wise it would onlyh iterate up to but not including the last element
        print(c,end ='')  #print statement to print the characters and using the end paramenter so that the they all appear on the same line, '' makes the characters print directly beside eachother
        if firstHoriszontalChar == w: #if is a conditional statement, if condition in true it executes the next line
            print('\n',end='') #print statement moves to new line
    for verticalChar in range(1,h+1): #another for loop, varticalChar is a variable iterating through the height values, range 1 to h+1 since it would not iterate to last value otherwise
        if width%2 != 0: #check if width is odd 
            gapnum = w//2 + 1 #odd= divide width by 2 and add 1,gaps desired
            gapstring =' '*gapnum #multiply gaps (empty string) by number of gaps desired(gapnum)
        else: #width is even
            gapnum = w//2 #even = divide width by 2,gaps desired
            gapstring = ' '*gapnum #multiply gaps (empty string) by number of gaps desired(gapnum)
        print(c + gapstring+ c) #print statement, concatenate character + gap + character each time
    for lastHoriszontalChar in range(1,w+1): #bottom horizontal frame
        print(c,end ='') # same as firstHoriszontalChar except we don't need to worry about moving to a newline as the frame is made
